module Spree
  module Admin
    class ProductsController < ResourceController
      ...
      # add here: generate SKU

      def new
        new_sku = generate_a_sku
        while Spree::Variant.where("sku LIKE '#{new_sku}' ").present?  # where("sku LIKE #{new_sku}%")
          new_sku = generate_a_sku 
        end
      ...
      protected
        ...
        def generate_a_sku
          return Date.today.year.to_s.slice!(2..3) + Date.today.day.to_s + Time.now.hour.to_s + Time.now.sec.to_s 
        end
