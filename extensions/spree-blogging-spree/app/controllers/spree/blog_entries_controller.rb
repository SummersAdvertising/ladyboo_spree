class Spree::BlogEntriesController < Spree::StoreController
  ...
  private
    ...
    def init_pagination
      ...
      @pagination_per_page = params[:per_page].to_i > 0 ? params[:per_page].to_i : 3  # 10
      @news_pagination_page = params[:news_page].to_i > 0 ? params[:news_page].to_i : 1
      @news_pagination_per_page = params[:news_per_page].to_i > 0 ? params[:news_per_page].to_i : 3  # 10
      @blog_pagination_page = params[:blog_page].to_i > 0 ? params[:blog_page].to_i : 1
      @blog_pagination_per_page = params[:blog_per_page].to_i > 0 ? params[:blog_per_page].to_i : 3  # 10
    end
end
